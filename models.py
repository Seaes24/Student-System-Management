...
class ProgramValidator:
    ...
    def validate_program_code(code):
        ...
        return bool(re.match(pattern, code)), "Valid" if re.match(pattern, code) else "Invalid format"
...
class DataLookup:
    ...
    def get_program_display_list(programs):
        ...
        """
        print(f"DEBUG - DataLookup received: {len(programs)} programs")  # Add this
        display_list = []
        for program in programs:
            print(f"DEBUG - Processing: {program}")  # Add this
        ...
        return sorted(display_list, key=lambda x: x['name'])  # Sort by program name